    ...
    <section class="declarations-page">
      <div class="declaration-card">
        <div class="card-header">
          <span>
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#6366f1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-file-text-icon lucide-file-text">
...
/**
 * Crée une déclaration pour TOUS les employés d'un employeur
...
function addEmployerDeclaration(employerId, date) {
  ...
  // Créer UNE DÉCLARATION PAR EMPLOYÉ
...
function script() {
  ...
  btn.onclick = () => {
    ...
    const declarations = addEmployerDeclaration(employerId, mois);
    
    if (declarations.length > 0) {
      toast.withLink(
        `${declarations.length} déclaration(s) réussie(s)`,
        "Voir l'historique",
        "/historique"
      );
